...
┃🤖 Ejemplo:
┃${usedPrefix + command} https://vm.tiktok.com/ZMFpeyxKa/
╰────────────⋆⋆`𝓟𝓸𝓵𝓑𝓸𝓽
     let ttdl = await tbTtdl(text)
    if (!args[0].match(/tiktok/gi)) throw `📵 ¿Está seguro que ese es un link de TikTok?`
    await m.reply(wait)
		
  if(command.includes('nowm')) {
      conn.sendFile(m.chat, ttdl.result.nowatermark, 'tiktok.mp4', `🤖 Tu amigo Polar ha conseguido este vídeo solo para ti.`.trim(), m)
   } else if (command.includes('audio')) {
     conn.sendFile(m.chat, ttdl.result.nowatermark, 'tiktok.mp3', null, m, null, { mimetype: 'audio/mp4' })
   } else {
     conn.sendHydrated(m.chat, `🤖 Tu amigo Polar ha conseguido este vídeo solo para ti`, igtb, ttdl.result.watermark, null, null, null, null, [['⎙ NOWM', `${usedPrefix}tiktoknowm ${text}`]], m)
   }
